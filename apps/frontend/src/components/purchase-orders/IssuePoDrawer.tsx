...
import { createPoIssueRequestAction } from '@/app/(app)/purchase-orders/actions';
...
const LOG = 'frontend:IssuePoDrawer';
...
function pdfErrorHint(message: string | null | undefined): string | null {
  ...
  return 'Fix the Purchase Order Word template in Document Templates, then retry.';
}
...
export interface IssuePoDrawerProps {
  open: boolean;
...
export function IssuePoDrawer({
  ...
  useEffect(() => {
    ...
    setSubject(poNumber ? `Purchase Order: ${poNumber}` : 'Purchase Order');
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      const result = await createPoIssueRequestAction(purchaseOrderId, {
      ...
      if (result.success) {
        toast.success(`Issuing PO to ${recipientsWithEmail.length} recipient${recipientsWithEmail.length === 1 ? '' : 's'}…`);
        onOpenChange(false);
        onSuccess?.();
      } else {
        setError(result.error ?? 'Failed to issue purchase order');
      }
    } catch (err) {
  ...
  return (
    <>
      <BottomFormDrawer
        ...
        title="Issue PO"
        description={poNumber ? `Issue Purchase Order ${poNumber}` : 'Issue Purchase Order'}
  ...
  return (
    <div className="space-y-4">
      <JobContactsPicker
        ...
        excludeIds={jobId ? [] : []}
        description="Search and select contacts to receive this purchase order. Each recipient must have an email address."
      ...
      <div className="rounded-lg border border-slate-200 bg-white px-4 py-4">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <Label htmlFor="po-issue-create-pdf" className="font-normal">
              Generate PDF
            </Label>
        ...
        <div className="flex flex-col items-center gap-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-12">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Generate a {formatLabel.toLowerCase()} to attach to the purchase order email.
            </p>
          </div>
...
function StepEmailPreview({
  ...
  const attachmentName = `PO${poNumber ? `-${poNumber}` : ''}.${docFormat === 'pdf' ? 'pdf' : 'docx'}`;
