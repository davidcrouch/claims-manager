...
export async function create(input: Input, _context: Context) {
  ...
  const promptText = requireString(input, "prompt_text");
  const tags = parseTags(input.tags);
  const version = parseVersion(input.version);

  if (!app || !name || !promptText) {
    return { error: `${LOG}:create - app, name, and prompt_text are required` };
  }
  ...
  try {
    ...
    const upsertParams = {
      ...
      methods: [
        {
          name: "render",
          description: "Render the prompt template with provided parameters",
        ...
        {
          name: "info",
          description: "Get prompt metadata",
...
export async function list(input: Input, _context: Context) {
  try {
    const client = await getClient();
    const registryCtx = getRegistryCtx(_context);
    const discoverParams: Record<string, unknown> = { type: "prompt" };
    const app = requireString(input, "app");
    ...
    return {
      prompts: result.capabilities.map((c: any) => ({
        cap: c.cap,
        name: c.name,
...
async function deletePrompt(input: Input, _context: Context) {
  const name = requireString(input, "name");
