...
import {
  fetchPoIssueRequestDetailAction,
  retryPoIssueRequestAction,
} from '@/app/(app)/purchase-orders/actions';
import type { PoIssueRequestDetail, PoIssueRequestRecipientDetail } from '@/lib/api-client';

const LOG = 'frontend:IssueBatchDetail';
...
export function IssueBatchDetail({
  purchaseOrderId,
  requestId,
  onBack,
}: {
  ...
  const [detail, setDetail] = useState<PoIssueRequestDetail | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await fetchPoIssueRequestDetailAction(purchaseOrderId, requestId);
  ...
  useEffect(() => {
    ...
    resolvePdfPreviewUrl(
      ...
      .catch((err) => {
        if (!cancelled) {
          const message =
            err instanceof Error ? err.message : 'Failed to load PO document';
  ...
  async function handleRetry() {
    ...
    setRetrying(true);
    try {
      const result = await retryPoIssueRequestAction(purchaseOrderId, requestId, {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="w-full min-w-0 space-y-1 text-sm sm:flex-1">
              ...
              {!detail.generatedDocId && (
                <p className="pt-1 text-xs text-muted-foreground">
                  No PO document is attached to this issue request.
                </p>
              )}
              {pdfError && (
              ...
              pdfLoading ? (
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-1 border-t border-slate-100 px-2 py-1.5">
                    <span className="truncate text-[11px] font-medium text-slate-700">PO.pdf</span>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-border/60">
                {detail.recipients.map((r: PoIssueRequestRecipientDetail) => (
      ...
      <BottomFormDrawer
        ...
        description="Document emailed with this issue request"
