// definitions/implementations/tools.ts
var _client = null;
...
function mapMethodDefs(raw) {
  if (!Array.isArray(raw)) return [];
  return raw.map((m) => ({
    name: String(m.name || ""),
    description: m.description ? String(m.description) : void 0,
    semantics: m.semantics ? String(m.semantics) : "work",
    inputSchema: m.input_schema ?? m.inputSchema ?? void 0,
    outputSchema: m.output_schema ?? m.outputSchema ?? void 0
  }));
}
...
async function list(input, _context) {
  try {
    const client = await getClient();
    const registryCtx = getRegistryCtx(_context);
    const discoverParams = { type: "tool" };
    const app = requireString(input, "app");
    ...
    const result = res.data;
    return {
      tools: result.capabilities.map((c) => ({
        cap: c.cap,
        name: c.name,
...
async function update(input, _context) {
  ...
  try {
    ...
    const methods = Array.isArray(input.methods) && input.methods.length > 0 ? mapMethodDefs(input.methods).map((m) => ({
      name: m.name,
      description: m.description,
      semantics: m.semantics,
      inputSchema: m.inputSchema,
...
async function deleteTool(input, _context) {
  const name = requireString(input, "name");
