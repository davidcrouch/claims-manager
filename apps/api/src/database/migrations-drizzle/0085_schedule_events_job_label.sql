...
-- Claims: received / lodged (not date of loss)
SELECT
  c.id,
  c.tenant_id,
  'claim'::text           AS event_type,
  COALESCE(c.claim_number, 'Claim') AS title,
  COALESCE(c.lodgement_date::timestamptz, c.created_at) AS starts_at,
  COALESCE(c.lodgement_date::timestamptz, c.created_at) AS ends_at,
  lv_c.name               AS status,
  NULL::text              AS priority,
  NULL::uuid              AS job_id,
  c.id                    AS claim_id
...
-- Jobs: request_date, else created_at
SELECT
  j.id,
  j.tenant_id,
  ...
  COALESCE(
    NULLIF(
      TRIM(BOTH FROM CONCAT_WS(
        ' · ',
        NULLIF(TRIM(j.internal_number), ''),
        NULLIF(TRIM(lv_jt.name), '')
      )),
      ''
    ),
    'Job'
  ) AS title,
  COALESCE(j.request_date::timestamptz, j.created_at) AS starts_at,
  COALESCE(j.request_date::timestamptz, j.created_at) AS ends_at,
  lv_j.name               AS status,
  NULL::text              AS priority,
  j.id                    AS job_id,
  j.claim_id
...
LEFT JOIN lookup_values lv_jt ON lv_jt.id = j.job_type_lookup_id
...
-- Quotes: estimated work date range, else quote_date / created_at
SELECT
  q.id,
  q.tenant_id,
  ...
  COALESCE(
    q.estimated_start_date::timestamptz,
    q.quote_date,
    q.created_at
  ) AS starts_at,
  COALESCE(
    q.estimated_completion_date::timestamptz,
    q.estimated_start_date::timestamptz,
    q.quote_date,
    q.created_at
  ) AS ends_at,
...
-- Invoices: issue / received date, else created_at
SELECT
  i.id,
  i.tenant_id,
  'invoice'::text         AS event_type,
  COALESCE(i.invoice_number, 'Invoice') AS title,
  COALESCE(i.issue_date, i.received_date, i.created_at) AS starts_at,
  COALESCE(i.issue_date, i.received_date, i.created_at) AS ends_at,
  lv_i.name               AS status,
  NULL::text              AS priority,
  i.job_id,
  i.claim_id
...
-- Proposals: proposal / received date, else created_at
SELECT
  p.id,
  p.tenant_id,
  'proposal'::text        AS event_type,
  COALESCE(p.name, p.proposal_number, 'Proposal') AS title,
  COALESCE(p.proposal_date, p.received_date, p.created_at) AS starts_at,
  COALESCE(p.proposal_date, p.received_date, p.created_at) AS ends_at,
  lv_p.name               AS status,
  NULL::text              AS priority,
  p.job_id,
  p.claim_id
