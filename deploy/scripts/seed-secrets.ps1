# deploy/scripts/seed-secrets.ps1
#
# Idempotently populates Secret Manager with the full set of secrets Cloud Run
# services read at boot. Run this once after `terraform apply` in the target
# environment has created the empty secret shells. Re-running is safe — secrets
# that already have a version are skipped unless -Force is set.
#
...
#   3. External placeholders (must be replaced by a human before the environment
...
#
# Usage:
#   pwsh deploy/scripts/seed-secrets.ps1 -Environment staging
#   pwsh deploy/scripts/seed-secrets.ps1 -Environment production
#   pwsh deploy/scripts/seed-secrets.ps1 -Environment production -Force
#   pwsh deploy/scripts/seed-secrets.ps1 -Environment staging -DryRun
...
function TfOut {
   param([string]$Key)
   $node = $tf.$Key
   if (-not $node) { Die "terraform output '$Key' is missing - redeploy environments/$Environment first" }
   return $node.value
}
...
function Test-SecretHasVersion {
   param([string]$Name)
   $v = & gcloud secrets versions list $Name --project=$Project --limit=1 --format='value(name)' 2>$null
   return [bool]$v
}
...
function Set-SecretValue {
   ...
   if (-not (Test-SecretExists $Name)) {
      Warn "secret '$Name' does not exist in $Project - did terraform apply module.secrets run? skipping"
   ...
   try {
      ...
      & gcloud secrets versions add $Name --project=$Project "--data-file=$tmpPath" | Out-Null
...
# OIDC_COOKIES_KEYS must parse as a JSON array.
...
# JWKS RSA (7 components) + EC (3 components).
...
function New-GcsHmacKey {
   param([string]$ServiceAccountEmail, [string]$GcpProject)

   if ($DryRun) { Log "plan  create HMAC key for $ServiceAccountEmail"; return $null }

   Log "creating GCS HMAC key for $ServiceAccountEmail"
   $raw = & gcloud storage hmac create $ServiceAccountEmail --project=$GcpProject --format=json 2>&1
...
if (-not ((Test-SecretHasVersion 'gcs-hmac-access-key') -or (Test-SecretHasVersion 'gcs-hmac-secret-key')) -or $Force) {
   $hmac = New-GcsHmacKey -ServiceAccountEmail $frontendSa -GcpProject $Project
   if ($hmac) {
...
$placeholders = @(
   @{ Name = 'nats-url';                       Hint = 'Not used currently. Replace only if NATS is introduced.' }
   ...
   @{ Name = 'stripe-secret-key';              Hint = 'Stripe dashboard - live mode key for production (test mode for staging)' }
   ...
   @{ Name = 'more0-api-key';                  Hint = 'More0 gateway API key' }
   @{ Name = 'more0-tool-secret';              Hint = 'More0 tool auth shared secret' }
   @{ Name = 'resend-api-key';                 Hint = 'Resend transactional email API key' }
)
...
foreach ($p in $placeholders) {
   ...
   try {
      ...
      & gcloud secrets versions add $p.Name --project=$Project "--data-file=$tmpPath" | Out-Null
...
if ($script:Manual.Count -gt 0) {
   ...
   Write-Host "  gcloud secrets versions add <NAME> --project=$Project --data-file=-" -ForegroundColor Yellow
}
Write-Host ''
Write-Host "Next:" -ForegroundColor Green
Write-Host "  1. Replace REPLACE_ME placeholders above (if any)."
Write-Host "  2. Deploy Cloud Run services so they pick up the new secret versions."
