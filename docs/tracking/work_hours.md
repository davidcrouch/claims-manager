...
- `2026-04-03` `eaea05a` **48 h**
  `440 files | +130 398 ‚àí0 | Tier 2 standard / Tier 3 complex | Heavy orchestration`
  Lay summary: First delivery of the claims management platform‚Äîsign-in, shared building blocks, quality checks before release, and repeatable environments so the team can keep building.
...
- `2026-04-03` `0185049` **0.5 h**
  `34 files | +3 ‚àí587 | Tier 1 mechanical | Light orchestration`
  Lay summary: Housekeeping only‚Äîauto-generated build folders are no longer kept in the change history, so records stay clean and easier to review.
  ...
  **No change** to product features or end-user behavior‚Äîthis is housekeeping that keeps reviews and clones professional.
  Reduces noise for anyone auditing what actually shipped in a given change.

- `2026-04-03` `bf394b2` **3 h**
  `1 file | +27 ‚àí289 | Tier 3 complex | Moderate orchestration`
...
- `2026-04-03` `ebf600e` **10.5 h**
  `19 files | +1 867 ‚àí11 290 | Tier 3 complex / Tier 4 deep integration | Heavy orchestration`
  Lay summary: Sign-in and ‚Äúwhich customer is this?‚Äù now line up across the website, services, and stored data, with a smoother path for people already on the product.
...
- `2026-04-07` `fa8b5f6` **38.5 h**
  `131 files | +7 685 ‚àí1 | Tier 2 standard | Heavy orchestration`
  Lay summary: Delivered the main claims management website‚Äîscreens for day-to-day work, login and logout, layout and navigation, and a live link to the supporting service behind it.
...
- `2026-04-07` `927ed6b` **4 h** (NOT-BILLABLE)
  `5 files | +404 ‚àí0 | Tier 3 complex | Moderate orchestration`
  Lay summary: Internal time-tracking and ledger lines tied to commits, including backfilled history‚Äîwork kept in-house for process, not billed to the client.
...
- `2026-04-10` `29ea32a` **10 h**
  `59 files | +7 723 ‚àí976 | Tier 3 complex | Heavy orchestration`
...
- `2026-04-13` `f12daa7` **5.5 h**
  `30 files | +5 592 ‚àí2 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-04-14` `f27ea49` **10 h**
  `65 files | +3 249 ‚àí56 | Tier 3 complex | Heavy orchestration`
  Lay summary: Repeatable path from automated checks through staging to live hosting, with packaged releases, staff runbooks, and each customer‚Äôs partner activity kept apart from others‚Äô.
...
- `2026-04-17` `fc9153a` **18 h**
  `98 files | +12 755 ‚àí823 | Tier 3 complex / Tier 4 deep integration | Heavy orchestration`
  ...
  **Delivered webhook pipeline v2** ‚Äî a coordinated path for bringing in partner-system activity so events are processed reliably end-to-end.
...
- `2026-04-18` `a1f5a74` **6 h**
  `47 files | +8 775 ‚àí2 374 | Tier 2 standard / Tier 3 complex | Heavy orchestration`
...
- `2026-04-18` `1d41703` **4 h**
  `17 files | +7 242 ‚àí36 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-04-20` `9ba736d` **14 h**
  `52 files | +2 617 ‚àí243 | Tier 3 complex / Tier 4 deep integration | Heavy orchestration`
  ...
  **Automated delivery pipeline to branlamie.com staging, plus live pipeline hardening.** Delivered an **end-to-end automated hosted delivery pipeline** so every change flows from automated checks through build, packaging, and rollout to the live staging site.
  Provisioned the staging environment as **infrastructure-as-code** ‚Äî databases, in-memory cache, private network, container registry, a hosted virtual machine, custom domains, and HTTPS ‚Äî with repeatable apply and rollback.
  ...
  Walked the pipeline through successive staging runs, **diagnosing and fixing each failure** surfaced by the live environment ‚Äî package manager pinning, first-time image bootstrap, migration networking, database name alignment, and standards-compliant token exchange.
...
- `2026-04-22` `45d0d54` **3 h**
  `17 files | +245 ‚àí195 | Tier 2 standard / Tier 4 deep integration | Heavy orchestration`
  ...
  **Staging authentication fix and Server Components resilience.** Diagnosed the root cause of the generic error message users were seeing on the staging site ‚Äî the **signing-key lookup address** was wrong, so every signed-in request was being rejected.
...
- `2026-04-22` `2f01230` **4.5 h**
  `2 files | +1 107 ‚àí33 | Tier 2 standard | Heavy orchestration`
...
- `2026-04-22` `b093287` **6 h**
  `75 files | +12 247 ‚àí671 | Tier 3 complex / Tier 4 deep integration | Heavy orchestration`
  ...
  **More0 webhook-workflow app with S3 archive, webhook tools, and event orchestration.** Packaged the webhook pipeline as a self-contained **More0 workflow app** (`process-inbound-event`) that takes an event identifier and drives the end-to-end flow: read the event, archive the raw payload, upsert mapped records, and update the processing log.
  ...
  Wired the **deployment layer** ‚Äî Docker Compose for local, Kubernetes external-secrets and api-server manifests, and the Terraform secrets module ‚Äî so the new bucket, More0 config, and environment toggles are available in every environment.
...
- `2026-04-22` `ff299f5` **4 h**
  `25 files | +2 599 ‚àí557 | Tier 2 standard | Moderate orchestration`
  Lay summary: Rebuilt every list screen ‚Äî claims, jobs, invoices, quotes, purchase orders, reports, and vendors ‚Äî around a shared, consistent layout with the same filters, search, and table behaviour so the application feels like one coherent product.
...
- `2026-04-23` `96dbedc` **2 h**
  `45 files | +3 887 ‚àí960 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-04-23` `762bed1` **5.5 h**
  `39 files | +574 ‚àí689 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-04-23` `d38ac0e` **5 h**
  `62 files | +998 ‚àí620 | Tier 2 standard | Moderate orchestration`
  ...
  **List/detail page header unification and request-routing rename.** Introduced a shared **list page header** and **set-page-header** building blocks and applied them consistently across claims, jobs, invoices, quotes, purchase orders, reports, vendors, and connections so every screen opens with the same title, breadcrumb, and action layout.
  Refreshed the **application shell** ‚Äî side navigation, top bar, layout wrapper, and breadcrumb provider ‚Äî so navigation feels cohesive across the product.
...
- `2026-04-23` `597f91a` **1 h**
  `21 files | +496 ‚àí270 | Tier 2 standard | Moderate orchestration`
...
- `2026-04-24` `dd3cb04` **4.5 h**
  `13 files | +1 719 ‚àí419 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-04-24` `ebbb86c` **6.5 h**
  `15 files | +15 373 ‚àí147 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
  ...
  **Schema alignment for quotes and purchase orders; quote / PO detail screens expanded; invoices-by-job lookup.** Added the missing structural pieces (external-reference keys, parent-cascade deletes, lookup links, child indexes) so quote groups, combos, items and purchase-order groups, combos, items match the partner-system contract end-to-end.
  ...
  Reworked the **quote detail** and **purchase-order detail** screens to present every contract field ‚Äî identity, parties, parent job and claim, service window, totals, adjustments, and payload fallback ‚Äî in clearly labelled sections.
...
- `2026-04-25` `99af045` **5.5 h**
  `24 files | +1 292 ‚àí719 | Tier 2 standard / Tier 3 complex | Moderate orchestration`
...
- `2026-05-13` `ddc2b5d` **8 h**
  ...
  **Operations modules, navigation, dashboard, tasks, sample data, staging notes, and work-hours artifacts.** Delivered **list and placeholder experiences** for work orders, requests for quote, proposals, bills, and accounts payable / receivable so teams can open consistent screens while deeper workflows mature.
  ...
  Grew the **service layer and stored shapes** for those recordsóincluding a database migration and richer sample dataóso the application reads from one coherent model.
...
- `2026-06-08` `820d7a2` **22 h**
  ...
  **Comprehensive UI audit and implementation of all outstanding specification gaps.** Audited every page against its design specification and addressed every identified gap across the full product surface.
  ...
  Created and improved **creation forms** ó new Task form, and extended the Quote and Invoice forms with all specified fields ó and wired them to standalone list pages and detail tabs.
...
- `2026-06-08` `8269b5f` **13.5 h**
  ...
  **Upstream UI observation specs and EnsureOS implementation specifications for the full product surface.** Walked through every page of the upstream Crunchwork Pulse vendor portal ó claims, jobs, quotes, purchase orders, invoices, and activities ó and recorded field layouts, navigation paths, status flows, and action buttons into a structured reference set.
  ...
  Captured the recursive contractor-chain concept ó customers upstream, vendors downstream ó so every module knows which direction data flows.
...
- `2026-06-15` `ddc250a` **38 h**
  ...
  **Catalogue module, domain layer, quote line-item integration, webhook proxy, and implementation specifications.** Delivered a full **product and service catalogue** with categories, item types, assemblies, pricing, CSV import, and an admin workspace for browsing, creating, and maintaining catalogue records.
  Built the **domain layer** ó transformers, use-case projections, shared services, workflow engine, document issuance, and outbound sync ó so jobs, quotes, purchase orders, and related records stay consistent as data flows through the application.
...
- `2026-06-19` `a78f00b` **20.5 h**
  ...
  **Quote line-item groups, assembly bill-of-materials expansion, and provider-scoped lookups.** Added **create, edit, delete, and reorder** flows for quote line-item groups with label lookups, descriptions, and dimensions surfaced in the quote detail workspace.
  ...
  Validated end-to-end quoting flows ó group management, assembly expansion, and catalogue pick-and-drop ó against sample building-repairs data.
...
- `2026-06-21` `96497e6` **24 h**
  ...
  **Journals module, appointments overhaul, and multi-screen job workspace improvements.** Delivered a full **journals feature** ó database schema, API, file storage for page attachments, list and detail screens, and a journals tab on each job.
...
- `2026-06-21` `e50a92f` **12 h**
  ...
  **List-page upgrade with sortable columns, active/archived tabs, and new entity create forms.** Replaced the previous sort-tab and status-filter controls with clickable sortable column headers and active/archived/all tabs across all list screens ó claims, jobs, invoices, quotes, reports, tasks, bills, proposals, purchase orders, RFQs, and work orders.
...
- `2026-06-30` `9393b56` **32 h**
  ...
  **Multi-catalog management, notifications, schedule, RFQ workflow, and Crunchwork integration sync.** Delivered **multi-catalog administration** ó create and edit catalogues, import line items from spreadsheets, browse assemblies, and pick catalogue items from quotes and jobs.
  ...
  Strengthened **Crunchwork provider sync** ó vendor sync service, job connection backfill, attachment downloads, connection documentation links, webhook handling, and mapper/transformer updates for jobs, quotes, purchase orders, and attachments.
...
- `2026-07-30` `1a6026f` **24.5 h**
  ...
  **Direct (claim-optional) jobs, internal job types, and list-filter upgrades.** Allowed jobs to exist without a linked claim and added an optional job display name.
  Seeded **internal job types** for create-job flows that are not tied to the external providerís type list.
...
- `2026-08-02` `dd4589c` **16 h**
  ...
  Lay summary: Document creation is smootheróstaff can upload files with previews, generate Word documents from templates, and convert Office files when needed.
...
- `2026-08-02` `c48cdab` **30 h**
  ...
  **Auth-server role and invitation upgrade with Cloud Run startup fixes.** Extended the **sign-in service** with role definitions, feature flags, permission administration, and invitation flows aligned to the organisationís parity target.
  ...
  Fixed **Cloud Run environment wiring** ó removed a reserved port setting and corrected private-network attachment so staging services start without platform errors.
...
- `2026-08-02` `b676b82` **0.5 h**
  ...
  Lay summary: Allowed the staging cloud services to accept traffic again after the organisationís access policy was updated.
...
- `2026-08-02` `4ecff44` **1 h**
  ...
  Lay summary: Logout pages now meet the siteís security script rules, so signed-out users are redirected without browser warnings.
...
- `2026-08-02` `038dff4` **10.5 h**
  ...
  Lay summary: Hosting is now Cloud Run onlyóold virtual-machine, Compose, and Kubernetes paths were removed so there is one supported way to run staging and production.
...
- `2026-08-08` `f8692c3` **38 h**
  ...
  Lay summary: Staff can keep company documents separate from each jobís files, choose filing templates when setting up the organisation and creating jobs, and keep clearer site journals.
  **Company and project document libraries, richer site journals, and smoother first-time setup.** Organisations now have a company filing area plus a separate project filing area for each job, chosen from templates during setup and when a job is created.
  The documents workspace shows company files and all projects together, or a single jobís project files when that job is in focus.
...
- `2026-08-09` `43c5468` **8 h**
  ...
  **Operations inbox, catalogue scopes, job assignment, and richer quote line items.** The home dashboard is a morning worklist of active jobs, items needing a decision, todayís schedule, and unread notices, each linking to the right screen.
...
- `2026-08-12` `3b9c62c` **1 h**
  ...
  **Fixed login and API routing so staff can sign in again.** Browser requests to the appís own API routes were being blocked by a framework middleware mismatch.
