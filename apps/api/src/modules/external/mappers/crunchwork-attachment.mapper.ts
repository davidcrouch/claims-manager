...
export class CrunchworkAttachmentMapper implements EntityMapper {
  ...
  async map(params: {
    ...
    this.logger.log(`CrunchworkAttachmentMapper.map — externalObjectId=${externalObjectId}`);
    ...
    const attachmentData = {
      ...
      relatedRecordId: relatedRecordId ?? '00000000-0000-0000-0000-000000000000',
      title: payload.title as string ?? payload.fileName as string ?? undefined,
      ...
      fileUrl: payload.downloadUrl as string ?? payload.fileUrl as string ?? undefined,
      apiPayload: payload,
      updatedAt: new Date(),
    };
    ...
    if (existingLink) {
      ...
      return { internalEntityId: existingLink.internalEntityId, internalEntityType: 'attachment' };
    }
