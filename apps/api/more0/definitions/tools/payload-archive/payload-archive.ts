...
/**
 * Sandbox tool: payload-archive
 * Calls POST /api/v1/webhook-tools/payloads/archive on the claims-manager API.
 */
export async function execute(
  input: Input,
  context: Context,
): Promise<Record<string, unknown>> {
  const logPrefix = 'payload-archive.execute';
  ...
  if (
    !isString(tenantId) ||
    !isString(providerEntityType) ||
    !isString(providerEntityId) ||
    !isObject(payload)
  ) {
    return {
      error: `${logPrefix} — invalid input: tenantId, providerEntityType, providerEntityId, payload required`,
    };
  }
  ...
  const url = `${baseUrl.replace(/\/$/, '')}/api/v1/webhook-tools/payloads/archive`;
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      providerEntityId,
      payload,
      hash: isString(hash) ? hash : undefined,
    }),
  });
  if (!res.ok) {
