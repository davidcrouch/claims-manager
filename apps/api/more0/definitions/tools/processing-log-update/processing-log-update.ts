...
/**
 * Sandbox tool: processing-log-update
 * Calls POST /api/v1/webhook-tools/processing-log/update on the claims-manager API.
 */
export async function execute(
  input: Input,
  context: Context,
): Promise<Record<string, unknown>> {
  const logPrefix = 'processing-log-update.execute';
  if (!isString(input['status'])) {
    return { error: `${logPrefix} — invalid input: 'status' required (non-empty string)` };
  }
  ...
  const url = `${baseUrl.replace(/\/$/, '')}/api/v1/webhook-tools/processing-log/update`;
