...
export function QuotesListClient({ initialData }: QuotesListClientProps) {
  ...
  useEffect(() => {
    ...
    if (lastFetchedSearch.current !== debouncedSearch) {
      ...
      fetchQuotesAction({}).then((res) => res && setData(res));
    }
  ...
  return (
    ...
    >
      ...
      {data.data.length === 0 && (
        <p className="py-8 text-center text-muted-foreground">No quotes found.</p>
      )}
    </EntityPanel>
  );
}
