...
export default function createAdminRoleRoutes(app: Application): void {
  /**
   * PUT /admin/users/:userId/roles — atomic replace of a user's org roles.
   */
  ...
  /**
   * GET /admin/users/:userId/roles — list active roles for a user.
   */
  ...
  /**
   * GET /admin/roles — list all role definitions.
   */
  app.get('/admin/roles', requireAuth(), async (req: Request, res: Response) => {
    try {
      if (!checkPermission(req, res, 'org.roles.read')) return;
