...
export class CrunchworkInvoiceMapper implements EntityMapper {
  ...
  async map(params: {
    ...
    this.logger.log(`CrunchworkInvoiceMapper.map — externalObjectId=${externalObjectId}`);
    ...
    const purchaseOrderId = await this.resolveFK({
      ...
      providerEntityId: (payload.purchaseOrder as Record<string, unknown>)?.id as string,
    ...
    const invoiceData = {
      ...
      issueDate: payload.issueDate ? new Date(payload.issueDate as string) : undefined,
      receivedDate: payload.receivedDate ? new Date(payload.receivedDate as string) : undefined,
    ...
    if (existingLink) {
      ...
      return { internalEntityId: existingLink.internalEntityId, internalEntityType: 'invoice' };
    }
