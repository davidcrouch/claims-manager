...
// definitions/implementations/web.e2e.test.ts
describe("tool.os.web (e2e with DuckDuckGo)", () => {
  ...
  describe("search (live DuckDuckGo)", () => {
    it("returns valid response shape (query, results array, total); when DuckDuckGo returns results, each has title/url/snippet", async () => {
      ...
      globalExpect(result).toHaveProperty("query", "Node.js LTS schedule");
      globalExpect(result).toHaveProperty("results");
      globalExpect(Array.isArray(result.results)).toBe(true);
      globalExpect(result).toHaveProperty("total");
      globalExpect(typeof result.total).toBe("number");
      const results = result.results;
      for (const r2 of results) {
        globalExpect(r2).toHaveProperty("title");
        globalExpect(r2).toHaveProperty("url");
        globalExpect(r2).toHaveProperty("snippet");
        globalExpect(r2.url).toMatch(/^https?:\/\//);
      }
    }, 15e3);
  });
  describe("fetch (live HTTP)", () => {
    it("fetches a real URL and returns status, body, headers", async () => {
      ...
      globalExpect(result).not.toHaveProperty("error");
      globalExpect(result).toHaveProperty("status", 200);
      globalExpect(result).toHaveProperty("body");
      globalExpect(typeof result.body).toBe("string");
      globalExpect(result.body.length).toBeGreaterThan(0);
      globalExpect(result).toHaveProperty("headers");
      globalExpect(result).toHaveProperty("truncated", false);
    }, 15e3);
  });
});
