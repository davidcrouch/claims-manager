...
import { fetchPoIssueRequestsAction } from '@/app/(app)/purchase-orders/actions';
import { IssueBatchDetail } from './IssueBatchDetail';
import { IssuePoDrawer } from './IssuePoDrawer';
import type { PoIssueRequestListItem } from '@/lib/api-client';
...
export function IssuesTab({
  ...
  jobId,
  issueDrawerOpen,
  onIssueDrawerOpenChange,
}: {
  purchaseOrderId: string;
  poNumber?: string | null;
  jobId?: string | null;
  issueDrawerOpen: boolean;
  onIssueDrawerOpenChange: (open: boolean) => void;
}) {
  const [requests, setRequests] = useState<PoIssueRequestListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await fetchPoIssueRequestsAction(purchaseOrderId);
      if (result.success && result.data) {
        setRequests(result.data);
      } else {
        setError(result.error ?? 'Failed to load Issue history');
      }
    } catch (err) {
    ...
    return (
      <>
        ...
        <IssuePoDrawer
          ...
          onOpenChange={onIssueDrawerOpenChange}
          purchaseOrderId={purchaseOrderId}
          poNumber={poNumber}
          jobId={jobId}
      ...
      {loading ? (
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">No issues sent</CardTitle>
          </CardHeader>
          <CardContent>
